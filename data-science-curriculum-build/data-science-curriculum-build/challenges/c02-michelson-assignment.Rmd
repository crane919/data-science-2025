...
**Observations**: - Write your observations here! - (Your response here) - Why might your table differ from Michelson's? - (Your response here)

Most of the recorded data is distinctness 3, and there is less distinctness 1 data. That makes sense because poor data measurement would be avoided if possible. The mean velocities are pretty similar between the three categories and especially between 3 and 2. Category 1 is slightly slower, but there is also less data points which could account from it being farther from 3 and 2.

My table might differ from Michelson's because of rounding or significant figures. Since Distinctiveness 3 and 2 on Michelson's have the same value, we know that Michelson thought they were the same.
...
**Observations**: - Is Michelson's estimate of the error (his uncertainty) greater or less than the true error? - Michelson's actual error is greater than the Michelson's error estimate of 51; It's roughly 3 times bigger.

\- Make a quantitative comparison between Michelson's uncertainty and his error. - The actual error is 2.97141176471 more than Michelson's error estimate.
...
**Observations**: Similarities - The mean and the positive and negative data values are roughly the same. The mean taken for each day tends to fall between the two dashed lines. Additionally most of the means except 2-3 fall between the dashed lines for both real and simulated data.

Differences - There are more above the dotted lines data values in the beginning of the table in the real data compared to the simulated. There are more data points below the dotted line around Jun 16th - Jun 23rd.

Overall I would say the data is more similar than different, since the means are very similar.
...
-   Graph #1: There is a very slight trend towards higher temperatures leading to a higher velocity. However, the data has high spread across the temperatures, making this an extremely weak trend. Temperature also doesn't appear to affect Distinctness in this graph as all three levels of distinctness happen throughout the temperature readings.
-   Graph #2: This graph shows the temperature vs the date, with each data point being colored for distinctness. It appears that the distinctness is related to the day the data was collected, as most days have all the same level of distinctness and there is never a day where a data recording of 1 and 3 appear together. This could be because the set up of the experiment was done better that day or different people were running the experiment on different days.
-   Graph #3: This graph shows that the more distinct the recording is the higher velocity it has, as the median value is the highest for 3 and lowest for 1. Distinctness 2 and 3 have a fairly similar range and median, while distinctness 1's velocity values are lower. That implies that distinctness level 2 data quality is good enough while distinctness level 1 is not.
