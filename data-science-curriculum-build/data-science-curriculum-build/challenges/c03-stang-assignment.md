...
- Is there “one true value” for the material properties of Aluminum?
  - No, because E and nu have different values across the different
    thickness and angles
- How many aluminum alloys are in this dataset? How do you know?
  - There is only one aluminum alloy tested: al_24st
- What angles were tested?
  - 0, 45, 90 degrees
- What thicknesses were tested?
  - 0.022, 0.032, 0.064, 0.081
- Is there any relationship between thickness and Poisson’s ratio of the
  aluminum alloy?
...
``` r
df_stang_long %>%
  ggplot(aes(x = thick, y = nu, color = angle)) +
  geom_count()
```
...
# df_stang_long %>%
#   ggplot() +
#   geom_point(aes(x = angle, y = E, color = thick))
# 
# df_stang_long %>%
#   ggplot() +
#   geom_point(aes(x = angle, y = nu, color = thick))
```

**Observations**:

- For a thickness of .02 to 0.06 there doesn’t appear to be a
  relationship between thickness and nu. The values stay roughly between
  .33 and .32. However when the thickness is .08 the values read between
  .31 and .32, which is different range than the thinner values. This
  difference suggests that there is a relationship between thickness and
  nu, however this pattern isn’t found within the thicknesses 0.02 to
  0.06. Therefore there would need to be more data to back up this
  claim. The data needed is more thickness measurements as no clear
  trend emerges from only testing 4 thicknesses.
...
- Does this graph support or contradict the claim above?
  - The graph seems to loosely not support the claim that E is an
    intensive material property, as there is a positive diagonal trend.
    The thickness colors are also grouped together. However on closer
    inspection, the colors are grouped not in decreasing order. As from
    bottom to top the groups are 0.081, 0.032, 0.064, and 0.022.
    Additionally the inclusion of nu is confusing as it is not part of
    the question being asked and impacts the shape of the data.
    Therefore the graph doesn’t actually support the claim.
- Is this evidence *conclusive* one way or another?
  - The evidence isn’t conclusive, because only 4 thicknesses were
    tested. It is hard to tell if anything is actually a pattern with
    only 4 thicknesses. Additionally within those 4 thicknesses there is
    a lot of variability and other variables are also changing (angle),
    so the values are have spread and no prominent pattern appears.
