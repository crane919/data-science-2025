...
-   Are there any sources of *real* uncertainty in the percent occupied you calculated?
    -   Yes, even though I counted the entire population of houses in Helvig, there can be uncertainty in that data as it isn't static. People can move and die, therefor changing the percentage over time.
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   No, because this just the first 25 houses that Zach found. It is likely last names will be clustered by houses, since people with the same last name tend to live together. This sample is not large enough or random enough to be representative of all the houses in Helvig.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    -   random
    -   Sequential will capture a singular neighborhood and not the entire population. For example family might chose to live near each other.
...
-   What is highest possible prevalence for your chosen name, based on the confidence interval you constructed?
    -   Based of the confidence interval the highest possible prevalence is .1896552 for the last name Collins
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   Bootstrap relies on an actual random sample, but sequential sampling isn't representative enough of the entire data
