...
df_q3 <- df_sample_seq %>%
  mutate(last_name = str_remove(name, "^.*\\s")) %>%
  group_by(last_name) %>%
  summarise(
    n = n(),
    p = n / nrow(df_sample_seq)
  ) %>%
  arrange(desc(p))                             
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - No, because this just the first 25 houses that Zach found. It is
    likely last names will be clustered by houses, since people with the
    same last name tend to live together. This sample is not large
    enough or random enough to be representative of all the houses in
    Helvig.
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - random
  - Sequential will capture a singular neighborhood and not the entire
    population. For example family might chose to live near each other.
...
- What is highest possible prevalence for your chosen name, based on the
  ...
  - Based of the confidence interval the highest possible prevalence is
    .1896552 for the last name Collins
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - Bootstrap relies on an actual random sample, but sequential sampling
    isn’t representative enough of the entire data
